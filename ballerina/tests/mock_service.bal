...
http:Service mockService = service object {
    ...
    resource function get assistants(@http:Header string OpenAI\-Beta) returns ListAssistantsResponse|http:BadRequest|error? {
    ...
    # http:BadRequest (The request has invalid or missing OpenAI-Beta header.)
    # error (An error occurred during processing.)
    resource function post assistants(@http:Payload CreateAssistantRequest requestBody, @http:Header string OpenAI\-Beta)
    returns AssistantObject|http:BadRequest|error? {
        if OpenAI\-Beta != "assistants=v2" {
            return {
                body: "Missing or invalid OpenAI-Beta header"
            };
    ...
    # error (An error occurred during processing.)
    resource function get assistants/[string assistantId](@http:Header string OpenAI\-Beta) returns AssistantObject|http:BadRequest|error? {
        if OpenAI\-Beta != "assistants=v2" {
            return {
                body: "Missing or invalid OpenAI-Beta header"
            };
    ...
    # http:BadRequest (The assistant is not found or the OpenAI-Beta header is invalid.)
    # error (An error occurred during processing.)
    resource function post assistants/[string assistantId](@http:Payload CreateAssistantRequest requestBody, @http:Header string OpenAI\-Beta)
    returns AssistantObject|http:BadRequest|error? {
        if OpenAI\-Beta != "assistants=v2" {
            return {
                body: "Missing or invalid OpenAI-Beta header"
    ...
    # http:BadRequest (The assistant is not found or the OpenAI-Beta header is invalid.)
    # error (An error occurred during processing.)
    resource function delete assistants/[string assistantId](@http:Header string OpenAI\-Beta) returns DeleteAssistantResponse|http:BadRequest|error? {
        if OpenAI\-Beta != "assistants=v2" {
            return {
                body: "Missing or invalid OpenAI-Beta header"
            };
    ...
    # error (An error occurred during processing.)
    resource function post threads(@http:Payload CreateThreadRequest requestBody) returns ThreadObject|http:BadRequest|error? {
    ...
    # http:BadRequest (The request is invalid.)
    # error (An error occurred during processing.)
    resource function post threads/runs(@http:Payload CreateThreadAndRunRequest requestBody) returns RunObject|http:BadRequest|error {
    ...
    # http:Ok (The request has succeeded.)
    # http:BadRequest (The request is invalid.)
    resource function post audio/speech(@http:Payload CreateSpeechRequest requestBody) returns byte[]|http:BadRequest {
    ...
    # http:BadRequest (The request is invalid.)
    # error (An error occurred during processing.)
    resource function post chat/completions(@http:Payload CreateChatCompletionRequest requestBody) returns CreateChatCompletionResponse|http:BadRequest|error? {
    ...
    # http:BadRequest (The request is invalid.)
    # error (An error occurred during processing.)
    resource function post completions(@http:Payload CreateCompletionRequest requestBody) returns CreateCompletionResponse|http:BadRequest|error? {
        string model = check requestBody.model.ensureType(string);
        return {
            id: "1",
            choices: [
    ...
    # http:BadRequest (The request is invalid.)
    # error (An error occurred during processing.)
    resource function post embeddings(@http:Payload CreateEmbeddingRequest requestBody) returns CreateEmbeddingResponse|http:BadRequest|error? {
        return {
            data: [
                {
                    index: 0,
